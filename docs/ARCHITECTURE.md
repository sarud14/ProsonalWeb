...
- CI: `.github/workflows/ci.yml` graph is `First-Notify-Discord` ∥ `Run-Node-Tests` → `E2E-Tests` → `Deploy-Notify-Discord` → `Vercel-Production` → `Second-Notify-Discord`. `check` is lint/type-check/unit/build; e2e uses Postgres + `prisma db push` + Playwright Chromium (admin stays ungated because `NEXTAUTH_SECRET` is unset). After tests pass, Actions runs `vercel pull` / `vercel build` / `vercel deploy --prebuilt` (`--prod` only on push to `main`; preview otherwise). `vercel.json` sets `git.deploymentEnabled: false` so Vercel Git does not also deploy that commit. Required GitHub secrets: `DISCORD_WEBHOOK_URL`, `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`. Discord helper: `.github/scripts/notify-discord.sh`.
