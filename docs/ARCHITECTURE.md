...
- **E2E:** Playwright in `e2e/`. `e2e/public-content.spec.ts` covers primary-nav → `/work` → open a case study, domain filter `aria-pressed`, `/login` heading, and contact submit (skipped without `DATABASE_URL`). `e2e/cms-publish-work.spec.ts` is the critical CMS flow: ungated `/admin/work/new` → Publish → public `/work/[slug]` 200, and Save draft → public slug 404. The spec skips unless `CONTENT_SOURCE=db` and `DATABASE_URL` are set, and skips if `/admin` redirects to OAuth login. Required work-form fields use `getByRole('textbox', { name: '<Label> *', exact: true })`; stack uses `getByRole('textbox', { name: 'Stack' })` from `TagInput` `ariaLabel`. CI e2e seeds from MDX after `prisma db push`.
...
- **CMS publish e2e is env-gated** — needs `CONTENT_SOURCE=db` + Postgres; OAuth login is not automated (spec skips if `/admin` redirects to `/login`).
