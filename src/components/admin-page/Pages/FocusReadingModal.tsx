...
import type { FocusReadingModalState } from '@/types/admin-pages.types'
...
interface FocusReadingModalProps {
  readonly state: FocusReadingModalState
  readonly onClose: () => void
  readonly onSave: (item: FocusReadingModalState['item']) => void
}

export function FocusReadingModal({
  state,
  onClose,
  onSave,
}: FocusReadingModalProps): React.JSX.Element {
    ...
    >
      ...
      >
        <h3 className="mb-5 text-[22px] font-medium">
          {state.isNew ? 'Add' : 'Edit'} reading item
        </h3>

        <div className="flex flex-col gap-4">
          <AdminFormField label="Index">
            <input
              className={adminInputClassName}
              value={item.idx}
              onChange={(e) => setItem((current) => ({ ...current, idx: e.target.value }))}
            />
          </AdminFormField>
