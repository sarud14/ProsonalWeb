import { CodeTypingBlock } from '@/features/landing/CodeTypingBlock'
...
import {
  pickHeroEngineerConfig,
  renderBodyWithHighlights,
} from '@/lib/landing/render-body-highlights'
...
export default function HeroSection({ hero }: HeroSectionProps): React.JSX.Element {
  const engineerConfig = pickHeroEngineerConfig(hero)
    ...
    <section className="pt-[88px] pb-[72px]">
      <Container className="grid max-w-[1240px] grid-cols-1 items-center gap-12 px-7 lg:grid-cols-[1.15fr_0.85fr]">
        <div>
          ...
          >
            <div className="mb-7 flex items-center gap-3">
              <span className="font-mono text-[11px] tracking-[0.16em] text-muted-foreground uppercase">
                <span className="text-primary">{'//'}</span> {hero.eyebrowLabel}
              </span>
              <LandingReveal
                trigger="mount"
          ...
          <LandingReveal trigger="mount" delay={LANDING_MOTION_HERO_TITLE_DELAY_MS}>
            <h1 className="text-[clamp(2.5rem,5.6vw,4.75rem)] font-semibold leading-[0.98] tracking-[-0.03em]">
              {hero.titleLine1}
              <br />
              {hero.titleLine2}
              <br />
              <span className="text-muted-foreground">{hero.titleLine3}</span>
            </h1>
          </LandingReveal>

          <LandingReveal trigger="mount" delay={LANDING_MOTION_HERO_BODY_DELAY_MS}>
            <p className="mt-7 max-w-[520px] text-[16.5px] leading-[1.65] text-muted-foreground">
              {renderBodyWithHighlights(hero.body, hero.bodyHighlights)}
            </p>
          </LandingReveal>

          <LandingReveal trigger="mount" delay={LANDING_MOTION_HERO_CTA_DELAY_MS}>
            <div className="mt-9 flex flex-wrap gap-3">
              <Button
                render={<Link href={hero.primaryCtaHref} />}
                nativeButton={false}
                size="lg"
          ...
          <div className="mt-11 flex flex-wrap gap-7">
            {hero.metaItems.map((item, index) => (
              <LandingReveal
                ...
                trigger="mount"
                delay={LANDING_MOTION_HERO_META_DELAY_MS + index * LANDING_MOTION_STAGGER_MS}
              >
                <div className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] tracking-[0.14em] text-muted-foreground uppercase">
                    {item.label}
                  </span>
                  <span className="text-[13.5px] text-secondary-foreground">{item.value}</span>
                </div>
              </LandingReveal>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-[18px]">
          ...
          >
            <Card className="relative rounded-none p-[13px]">
              ...
              <div className="relative flex h-[320px] w-full items-center justify-center overflow-hidden bg-background/40">
                {hero.profileImageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element -- CMS URLs may be any host (e.g. Vercel Blob)
                  <img
                    src={hero.profileImageUrl}
                    alt={hero.profileImageAlt || hero.profileName}
              ...
              <div className="mt-[13px] flex items-center justify-between">
                <div className="flex flex-col gap-[3px]">
                  <p className="text-sm font-semibold tracking-[-0.01em]">{hero.profileName}</p>
          ...
          >
            <CodeTypingBlock
              key={`${hero.codeFilename}-${hero.codeRole}-${hero.codeStack.join('-')}`}
